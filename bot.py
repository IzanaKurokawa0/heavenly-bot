...
# ========== ТЕЛЕГРАМ БОТ (старая версия API) ==========
...
async def rating(update: Update, context: CallbackContext, page=0):
...
async def show_club(update: Update, context: CallbackContext, club_id: int):
...
async def club_command_handler(update: Update, context: CallbackContext):
...
async def handle_search(update: Update, context: CallbackContext):
    ...
    if not results:
        await update.message.reply_text(f"🔍 По запросу '{query}' ничего не найдено", parse_mode=ParseMode.HTML)
        return
...
def run_bot():
    ...
    try:
        ...
        dp.add_handler(CommandHandler("rating", lambda u, c: async_handler(rating)(u, c, 0)))
        dp.add_handler(CommandHandler("search", async_handler(search_command)))
        dp.add_handler(CommandHandler("update", async_handler(update_command)))
        dp.add_handler(CommandHandler("help", async_handler(help_command)))
        dp.add_handler(CommandHandler("status", async_handler(status_command)))
        
        # Обработчики кнопок
        dp.add_handler(CallbackQueryHandler(async_handler(button_handler)))
        
        # Обработчик текста
        dp.add_handler(MessageHandler(Filters.text & ~Filters.command, async_handler(handle_search)))
