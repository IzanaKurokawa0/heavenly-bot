...
# ТВОЙ ТОКЕН ВСТАВЛЕН ПРЯМО В КОД
TELEGRAM_TOKEN = "8529987392:AAELJdw9sPpk4F2BiByLNPzPYoUAtwSVpuU"  # Твой токен
BRAWL_API_KEY = os.getenv('BRAWL_API_KEY', '')  # API ключ будет из переменных окружения
...
if not TELEGRAM_TOKEN or TELEGRAM_TOKEN == "8529987392:AAELJdw9sPpk4F2BiByLNPzPYoUAtwSVpuU":
    logger.info("✅ Telegram токен установлен напрямую в коде")
else:
    logger.warning("⚠️ Используется другой токен")

if not BRAWL_API_KEY:
    logger.warning("⚠️ BRAWL_API_KEY не установлен. Команда /update не будет работать")
    logger.info("💡 Чтобы /update работал, добавь BRAWL_API_KEY в Environment Variables Render")
else:
    logger.info("✅ Brawl Stars API ключ установлен")

# Файл данных для Render
DATA_FILE = 'clubs_data.json'
CLUBS_PER_PAGE = 10

logger.info(f"🚀 Бот запускается на Render.com с твоим токеном")
...
async def update_clubs_data():
    ...
    if not BRAWL_API_KEY:
        ...
        logger.error("✅ РЕШЕНИЕ: Добавь BRAWL_API_KEY в Environment Variables Render")
        return 0
    ...
    for i, club in enumerate(data['clubs']):
        try:
            ...
            logger.info(f"📡 Запрос данных для {club['name']} ({club['tag']})...")
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ {club['name']}: Ошибка 403 - IP не в белом списке!")
                logger.error(f"✅ РЕШЕНИЕ: Добавь IP Render.com в developer.brawlstars.com")
            elif response.status_code == 404:
                logger.warning(f"⚠️ {club['name']}: Клуб не найден (404)")
            else:
            ...
            logger.error(f"❌ {club['name']}: Таймаут запроса")
        except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    host_info = "🚀 Render.com (24/7)"
    
    text = f"""
<b>🏆 HEAVENLY DYNASTY BOT 🏆</b>
<i>Работает на {host_info}</i>
...
async def update_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        ...
        await update.callback_query.edit_message_text(
            "🔄 <b>Обновляю данные через API Brawl Stars...</b>\n\n"
            "• Использую Render.com со статическим IP\n"
            "• Это может занять до 30 секунд\n"
            "• Паузы между запросами 0.5с\n\n"
            "<i>Если ошибка 403 - добавь IP Render в developer.brawlstars.com</i>",
            parse_mode='HTML'
        )
    else:
    ...
    try:
        if not BRAWL_API_KEY:
            error_text = (
                "❌ <b>ОШИБКА: BRAWL_API_KEY не настроен</b>\n\n"
                "1. Зайди в Dashboard Render.com\n"
                "2. Выбери сервис heavenly-bot\n"
                "3. Environment → Add Environment Variable\n"
                "4. Добавь: BRAWL_API_KEY = твой_ключ_brawl_stars\n\n"
                "Без ключа команда /update не работает!"
            )
            if update.callback_query:
...
• Хостинг: <b>Render.com со статическим IP</b>
...
Возможные причины:
1. ❌ BRAWL_API_KEY неверный
2. ❌ IP Render.com не добавлен в белый список
3. ❌ Проблемы с API Brawl Stars

📋 <b>Проверь:</b>
1. developer.brawlstars.com - IP Render в белом списке?
2. Environment Variables - ключ правильный?
3. Логи Render - подробные ошибки

Текущее время: <b>{data['last_update']}</b>
            """
...
<code>/status</code> - Статус бота и хостинга
...
(Нужен BRAWL_API_KEY в Environment Variables Render)

🚀 <b>Хостинг Render.com:</b>
• Статический IP для API запросов
• Работа 24/7 бесплатно
• IP нужно добавить в developer.brawlstars.com
...
<i>Если /update не работает - проверь IP в белом списке!</i>
    """
...
🚀 <b>Хостинг Render.com:</b>
• Статус: <b>Активен ✅</b>
• Режим работы: <b>24/7 (засыпает при неактивности)</b>
• Telegram токен: <b>Установлен напрямую в коде ✅</b>
...
• Файл данных: <code>{DATA_FILE}</code>
• Telegram токен в коде: <b>Да ✅</b>
• Хостинг: <b>Render.com Free Tier</b>
...
"""
    ...
    if not BRAWL_API_KEY:
        text += "• Добавьте BRAWL_API_KEY в Environment Variables Render для работы /update\n"
    
    text += "• Бот работает даже без API ключа (статические данные)\n"
...
async def main():
    ...
    print("🚀 Запускается на Render.com с токеном в коде...")
    print("=" * 60)
    
    # Проверяем токен
    if not TELEGRAM_TOKEN or TELEGRAM_TOKEN == "8529987392:AAELJdw9sPpk4F2BiByLNPzPYoUAtwSVpuU":
        print("✅ Telegram токен установлен напрямую в коде")
    else:
        print(f"✅ Токен Telegram: {TELEGRAM_TOKEN[:10]}...")
    
    print(f"✅ Ключ Brawl Stars API: {'Установлен' if BRAWL_API_KEY else 'Нет'}")
    print(f"✅ Файл данных: {DATA_FILE}")
    
    # Проверяем что токен не дефолтный (на всякий случай)
    if TELEGRAM_TOKEN == "8529987392:AAELJdw9sPpk4F2BiByLNPzPYoUAtwSVpuU":
        print("⚠️  Используется твой реальный токен Telegram")
