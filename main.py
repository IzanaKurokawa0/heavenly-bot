...
if not BRAWL_API_KEY:
    logger.warning("⚠️  BRAWL_API_KEY не установлен в переменных окружения. Используется значение по умолчанию.")
    # Если ключ не установлен, используем тот, который у вас был ранее
...
async def health_check(request):
    ...
    return web.json_response({
        ...
        "telegram_bot": "running" if application else "not_running",
...
async def get_club_data(club_tag: str) -> Optional[Dict]:
    ...
    if not BRAWL_API_KEY or BRAWL_API_KEY == 'ваш_ключ_здесь':
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if BRAWL_API_KEY.startswith('eyJ'):
        ...
        api_status = "Пользовательский ключ"
    ...
    status_text += f"• Память: {memory.percent}% ({memory.used // (1024**3)}GB/{memory.total // (1024**3)}GB)\n"
    status_text += f"• Диск: {disk.percent}%\n"
    ...
    try:
        ...
        status_text += f"• Время работы: {days}д {hours}ч {minutes}м\n"
    except:
        pass
...
def main():
    ...
    env_vars = {
        ...
        "BRAWL_API_KEY": "установлен" if BRAWL_API_KEY and BRAWL_API_KEY != 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9.eyJpc3MiOiJzdXBlcmNlbGwiLCJhdWQiOiJzdXBlcmNlbGw6Z2FtZWFwaI' else "по умолчанию",
