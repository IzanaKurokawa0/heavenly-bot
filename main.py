...
async def start_background_tasks(app):
    ...
    await get_current_ip()
    # Проверяем статус API при запуске
    await check_api_status()
...
async def main():
    ...
    try:
        ...
        # Настраиваем веб-хук, если URL указан
        if WEBHOOK_URL:
            success = await setup_webhook(app)
            if success:
                logger.info(f"🌐 Режим webhook: {WEBHOOK_URL}")
            else:
                logger.error("❌ Не удалось установить webhook")
        else:
            logger.warning("⚠️ WEBHOOK_URL не установлен. Бот будет работать только через веб-хук.")
        ...
        # Если webhook настроен, просто держим приложение запущенным
        if WEBHOOK_URL:
            logger.info("🔄 Ожидаем входящие webhook запросы...")
            # Бесконечный цикл для поддержания работы
            while True:
                await asyncio.sleep(3600)  # Спим 1 час
        else:
            logger.info("⚠️ WEBHOOK_URL не установлен, бот не может получать обновления")
            # Ждем вечно, так как без webhook мы не можем получать сообщения
            while True:
