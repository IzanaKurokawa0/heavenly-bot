...
async def rating_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not clubs_data:
        await update.message.reply_text("❌ Не удалось загрузить данные клубов")
        return
...
async def init_bot():
    ...
    for club_name in CLUBS.keys():
        short = club_name.split()[-1].lower() if ' ' in club_name else club_name.lower()
        app.add_handler(CommandHandler(short, club_info_command))
...
async def create_app():
    """Создание aiohttp приложения для Render"""
    # Инициализируем бота
    await init_bot()
    ...
    if WEBHOOK_URL:
        ...
        logger.warning("⚠️ Режим без webhook, бот будет получать только команды через веб-хук")
    ...
    # Добавляем cleanup при завершении
    app.on_cleanup.append(lambda app: asyncio.create_task(cleanup()))
...
def main():
    ...
    if is_render:
        # Режим для Render: запуск через web.run_app
        logger.info("🚀 Запуск в режиме Render...")
        ...
        async def local_main():
            ...
            if not WEBHOOK_URL:
                logger.info("📡 Используется polling (для разработки)...")
                # Режим polling для локальной разработки
                await application.run_polling()
            else:
                logger.info("🌐 Используется webhook...")
                # Создаем и запускаем веб-сервер локально
